...
namespace T3D
{
	...
	void DrawTask::init() {
		...
		dish[0] = Vector3(312 + (cubeSize / 2), 620, 1);
		dish[1] = Vector3(312 + (cubeSize / 2), 620 - cubeSize, 1);
		dish[2] = Vector3(312 - (cubeSize / 2), 620 - cubeSize, 1);
		dish[3] = Vector3(312 - (cubeSize / 2), 620, 1);

		spoon[0] = Vector3(712 + (cubeSize / 6), 620, 1);
		spoon[1] = Vector3(712 + (cubeSize / 6), 620 - cubeSize, 1);
		spoon[2] = Vector3(712 - (cubeSize / 6), 620 - cubeSize, 1);
		spoon[3] = Vector3(712 - (cubeSize / 6), 620, 1);
	}
	...
	void DrawTask::drawFilledSemiCircle(int cx, int cy, int r, Colour c)
	{
		for (float y = 0; y <= r / sqrt(2); y++)
		{
			float x = sqrt(r * r - y * y);
	...
	void DrawTask::update(float dt) {
		...
		// Ground and horizon
		int horizon = 300;
		drawDDALine(312, 640, 512, horizon, Colour(150, 75, 0, 255));
		drawDDALine(712, 640, 512, horizon, Colour(150, 75, 0, 255));
		drawDDALine(0, horizon, 1024, horizon, Colour(150, 75, 0, 255));

		// Sky
		for (int i = 0; i < horizon; i++)
		{
			// Use this instead of one of the line functions so there's no branching to speed it up a bit
			for (int x = 0; x <= 1024; x++)
			{
				drawArea->plotPixel(x, i, Colour(210, 150, 20, 255));
			}
		}
		
		// Sun
		drawFilledSemiCircle(512, horizon - 1, 81, Colour(255, 255, 0, 255));

		// Draw coloured in "dish"
		for (int i = dish[1].y; i < dish[0].y; i++)
		{
			drawDDALine(dish[0].x, i, dish[3].x, i, Colour(255, 255, 255, 255));
		}

		// Draw coloured in "spoon"
		for (int i = spoon[1].y; i < spoon[0].y; i++)
		{
			drawDDALine(spoon[0].x, i, spoon[3].x, i, Colour(200, 200, 200, 255));
		}

		int dishTimeMultiple = 32;
		
		// Make the dish and the spoon run away into the sunset
		for (int i = 0; i < 4; i++)
		{
			if (timeElapsed <= 10)
			{
				dish[i] =
					translate(312 + floor(timeElapsed * dishTimeMultiple), 620 - (cubeSize / 2) - floor(timeElapsed * dishTimeMultiple)) *
					scale(0.996, 0.996) *
					translate(-312 - floor(timeElapsed * dishTimeMultiple), -(620 - (cubeSize / 2) - floor(timeElapsed * dishTimeMultiple))) *
					dish[i];

				spoon[i] =
					translate(712 - floor(timeElapsed * dishTimeMultiple), 620 - (cubeSize / 2) - floor(timeElapsed * dishTimeMultiple)) *
					scale(0.996, 0.996) *
					translate(-712 + floor(timeElapsed * dishTimeMultiple), -(620 - (cubeSize / 2) - floor(timeElapsed * dishTimeMultiple))) *
					spoon[i];
			}
